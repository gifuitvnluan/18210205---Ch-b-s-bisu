...
function pager($totalPage, $pageid, $pagerDispLength,$encodingType){
	...
	if($pageid != 1 ) {
		 $pager .= '<li><a href="?page='.$prev.$addParam.'">'.$pagerPrev.'</a></li>';
	}
	if($startPage>= 2){
		$pager .= '<li><a href="?page=1'.$addParam.'" class="btnFirst">1</a></li>';
		if($startPage>= 3) $pager .= '<span class="overPagerPattern">'.$overPagerPattern.'</span>'; //ドットの表示
	}
	for($i=$startPage; $i <= $endPage ; $i++){
		$class = ($pageid == $i) ? ' class="active"':"";//現在のページ番号にclass追加
		if($i <= $totalPage && $i> 0 )//1以上最大ページ数以下の場合
			$pager .= '<li><a href="?page='.$i.$addParam.'"'.$class.'>'.$i.'</a></li>';//ページ番号リンク表示
	}
	if($totalPage> $endPage){
		if($totalPage-1> $endPage ) $pager .= '<span class="overPagerPattern">'.$overPagerPattern.'</span>'; //ドットの表示
		$pager .= '<li><a href="?page='.$totalPage.$addParam.'" class="btnLast">'.$totalPage.'</a></li>';
	}
	if($pageid <$totalPage){
		$pager .= '<li><a href="?page='.$next.$addParam.'">'.$pagerNext.'</a></li>';
	}
...
function dspThumb($data,$width=100){
	...
	if($dspThumbNail == 1){
		if(isset($data['file_type'][0]) && isset($data['upfile_path_thumb'][0]) && $data['file_type'][0] == 'img' && file_exists($data['upfile_path_thumb'][0])){
			...
			$imgTag = $data['upfile_path_thumb'][0];
		}
	}
	return $imgTag;
}
